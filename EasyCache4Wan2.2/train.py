...
# -*- coding: utf-8 -*-
"""
Train a cumulative-risk predictor from EasyCache lazy-data trajectory files.
...
The stored scalar target is treated as a one-step local cache-error increment.
For a sample at row i, training constructs cumulative future risk labels:

    [target[i],
     target[i] + target[i+1],
     ...,
     sum(target[i:i+horizon])]

The model predicts positive risk increments and returns their cumulative sum.
Consequently, training, validation, and inference all use the same safe-prefix
contract: the longest prefix whose cumulative predicted risk stays below the
threshold.
...
class LazyHorizonPredictor(nn.Module):
    """Predict monotonic cumulative EasyCache risk over a future horizon."""
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        # Positive increments guarantee monotonic cumulative risk.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a cumulative EasyCache-risk predictor and evaluate the "
            "maximum prefix whose cumulative predicted risk stays safe."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/easycache_lazy_cumulative",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Complete EasyCache-error threshold used for prefix metrics.",
    )
    parser.add_argument(
        "--horizon-decay",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Penalty for underestimating a complete future cache error.",
    )
    parser.add_argument(
        "--unsafe-weight",
        type=float,
        default=4.0,
        help="Penalty when a true complete error is unsafe but prediction is safe.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.90,
        help="Upper quantile learned for cumulative risk prediction.",
    )
    parser.add_argument(
        "--near-threshold-weight",
        type=float,
        default=2.0,
        help="Additional regression weight for labels close to the threshold.",
    )
    parser.add_argument(
        "--threshold-bandwidth",
        type=float,
        default=0.02,
        help="Bandwidth of the near-threshold weighting kernel.",
    )
    parser.add_argument(
        "--calibration-coverage",
        type=float,
        default=0.95,
        help="Validation coverage used for one-sided conformal offsets.",
    )
    parser.add_argument(
        "--max-unsafe-rate",
        type=float,
        default=0.01,
        help=(
            "Checkpoint selection first satisfies this validation over-skip "
            "rate, then maximizes the mean safe-prefix length."
        ),
    )
...
def build_windows(
    ...
    for trajectory_id, trajectory in enumerate(trajectories):
        ...
        for start in range(n):
            ...
            increment_window = torch.zeros(horizon, dtype=torch.float32)
            ...
            for offset in range(horizon):
                ...
                increment_window[offset] = trajectory.targets[index]
            ...
            # Existing files contain one-step local error increments.  The
            # deployed policy budgets their cumulative sum, so supervision must
            # use exactly the same quantity.
            target_rows.append(torch.cumsum(increment_window, dim=0))
...
def loss_function(
    ...
    unsafe_weight: float,
    quantile: float,
    near_threshold_weight: float,
    threshold_bandwidth: float,
) -> torch.Tensor:
    ...
    del huber_beta  # Kept in the CLI for compatibility with earlier commands.

    residual = target - prediction
    pointwise_regression = torch.maximum(
        quantile * residual,
        (quantile - 1.0) * residual,
    )
    if near_threshold_weight > 0:
        proximity = torch.exp(
            -(target - threshold).abs() / threshold_bandwidth
        )
        pointwise_regression = pointwise_regression * (
            1.0 + near_threshold_weight * proximity
        )
    regression = masked_weighted_mean(pointwise_regression, mask, weights)

    pointwise_underestimation = F.relu(target - prediction).square()
    pointwise_underestimation = masked_weighted_mean(
        pointwise_underestimation,
        mask,
        weights,
    )

    true_unsafe = mask & (target >= threshold)
    if true_unsafe.any():
        unsafe_margin = F.relu(threshold - prediction).square()
    ...
    return (
        regression
        + underestimate_weight * pointwise_underestimation
...
def prefix_decision_metrics(
    ...
    error = predicted_prefix.to(torch.float32) - true_prefix.to(torch.float32)
    return {
        "prefix_mae": error.abs().mean().item(),
        "prefix_exact_accuracy": (
            predicted_prefix == true_prefix
        ).float().mean().item(),
        "unsafe_over_skip_rate": (
            predicted_prefix > true_prefix
        ).float().mean().item(),
        "conservative_rate": (
            predicted_prefix < true_prefix
...
def evaluate(
    ...
    for feature_batch, target_batch, mask_batch in loader:
        ...
        loss = loss_function(
            ...
            args.unsafe_weight,
            args.quantile,
            args.near_threshold_weight,
            args.threshold_bandwidth,
        )
...
def conformal_offsets(
    ...
    """Compute one-sided per-horizon residual offsets on validation data."""
    offsets: list[torch.Tensor] = []
    for offset in range(prediction.shape[1]):
        valid = mask[:, offset]
        residual = target[valid, offset] - prediction[valid, offset]
        if residual.numel() == 0:
            offsets.append(prediction.new_zeros(()))
            continue

        residual = residual.sort().values
        rank = math.ceil((residual.numel() + 1) * coverage) - 1
        rank = min(max(rank, 0), residual.numel() - 1)
        offsets.append(residual[rank].clamp_min(0.0))

    # Offsets can otherwise make the calibrated cumulative curve non-monotonic.
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.near_threshold_weight < 0:
        raise ValueError("near-threshold-weight must be non-negative")
    if args.threshold_bandwidth <= 0:
        raise ValueError("threshold-bandwidth must be positive")
    if not 0 < args.calibration_coverage < 1:
        raise ValueError("calibration-coverage must be in (0,1)")
    if not 0 <= args.max_unsafe_rate < 1:
        raise ValueError("max-unsafe-rate must be in [0,1)")
    if min(
        args.underestimate_weight,
        args.unsafe_weight,
    ) < 0:
...
def main() -> None:
    ...
    logging.info(
        "Cumulative target risks: train mean=%.6f std=%.6f min=%.6f max=%.6f | "
        "val mean=%.6f max=%.6f",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0:
            logging.info(
                ...
                "prefix_mae=%.4f | val loss=%.6f mae=%.6f "
        ...
        selection_key = (
            max(val_metrics.unsafe_over_skip_rate - args.max_unsafe_rate, 0.0),
            -val_metrics.mean_predicted_prefix,
            val_metrics.loss,
        )
    ...
    calibration_offsets = conformal_offsets(
        final_prediction,
        final_target,
        final_mask,
        args.calibration_coverage,
    )
    calibrated_prediction = torch.cummax(
        final_prediction + calibration_offsets.unsqueeze(0),
        dim=1,
    ).values
    ...
    # Add deployment calibration metadata without changing the pure state_dict
    # checkpoint used by older code.
    full_payload = safe_load(full_checkpoint_path)
    full_payload["calibration_offsets"] = calibration_offsets.cpu()
    full_payload["calibration_coverage"] = args.calibration_coverage
    full_payload["target_contract"] = "cumulative_sum_of_local_cache_errors"
    full_payload["model_output_contract"] = "monotonic_cumulative_risk"
    torch.save(full_payload, full_checkpoint_path)
    ...
    summary = {
        ...
        "calibration_coverage": args.calibration_coverage,
        "calibration_offsets": calibration_offsets.tolist(),
    ...
    logging.info(
        "Finished | best epoch=%d | val MAE=%.6f "
        "prefix_MAE=%.4f exact=%.4f unsafe_over_skip=%.4f",
        best_epoch,
        final_metrics.mae,
    ...
    logging.info(
        "Calibrated deployment prefix metrics: %s",
        calibrated_prefix_metrics,
    )
